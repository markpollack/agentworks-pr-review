...
**The four served artifacts are GENERATED. Do not hand-edit them.**
`src/main/resources/v3alpha/workflow-pr-review.json` is emitted from
`v3/PrReviewWorkflowV3`; the sibling artifact record, curated catalog, and immutable
catalog selection are derived from the same emission and provider beans. `PrReviewSpecV3Test`
byte-compares all four against a fresh emission.
...
- **Emission is a build-time act, on purpose.** Repository-relative node locations live in
  `workflow-artifact-record.json`, outside the producer-neutral workflow and its `specHash`.
  Emit in the checkout, review the four artifacts, and serve the exact generated bytes.
...
- `serving/WorkflowV3AlphaController` serves GET `/workflow/v3alpha/view`, `/catalog`,
  `/selection`, and `/artifact-record` for the ONE pr-review workflow. The view joins the
  exact artifact-record hash and carries source evidence for all 19 nodes.
